...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5 && newVersion >= 5) {
      // Add FBS, PPBS, and RBS columns to sugar_records table
      await db.execute('''
        ALTER TABLE ${AppConstants.sugarRecordsTable} ADD COLUMN fbs INTEGER
      ''');
      await db.execute('''
        ALTER TABLE ${AppConstants.sugarRecordsTable} ADD COLUMN ppbs INTEGER
      ''');
      await db.execute('''
        ALTER TABLE ${AppConstants.sugarRecordsTable} ADD COLUMN rbs INTEGER
      ''');
    }
  }
